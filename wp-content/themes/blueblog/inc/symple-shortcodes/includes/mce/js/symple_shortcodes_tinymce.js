		...
		createControl : function(btn, e) {
			if ( btn == "symple_shortcodes_button" ) {
				...
				var btn = e.createSplitButton('symple_button', {
					...
					image: sympleShortcodesVars.template_url +"/inc/symple-shortcodes/includes/mce/images/shortcodes.png",
					icons: false,
	            });
		...
		render : function(ed, title, id) {
			ed.add({
				...
				onclick: function () {
					...
					// Callout
					if(id == "callout") {
						tinyMCE.activeEditor.selection.setContent('[symple_callout button_text="button text" button_color="blue" button_url="http://www.wpexplorer.com" button_rel="nofollow"]Your caption goes here[symple_callout]');
					}
					...
					//Skillbar
					if(id == "skillbar") {
						tinyMCE.activeEditor.selection.setContent('[symple_skillbar title="WordPress" percentage="100" color="#6adcfa"]');
					}
